...
class Trainer:
    ...
    def train(
        ...
        for batch, actions in train_gen:
            # Fetch data
            observations = batch.cuda()
            observed_actions = actions.cuda()
            batch_size = observations.size(0)
            num_observations = self.config["num_observations"]
            observations = observations[:, :num_observations]
            observed_actions = observed_actions[:, :num_observations]

            # Forward the model
            model_outputs = model(observations, observed_actions)
            ...
            if self.global_step % media_logging_frequency == 0 and self.is_main_process:
                model_outputs["generated_observations"], _ = dmodel.generate_frames(
                    observations=observations[:min(4, batch_size), :self.config["condition_frames"]],
                    actions = observed_actions[:min(4, batch_size), :self.config["condition_frames"]],
