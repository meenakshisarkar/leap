...
class Model(nn.Module):
    ...
    def forward(
            self,
            observations: torch.Tensor, actions: torch.Tensor) -> DictWrapper[str, Any]:
        """
        ...
        unsqueezed = actions.unsqueeze(-1).unsqueeze(-1)  
        action_maps = unsqueezed.repeat(1, 1, 1, 4, 4)
        ...
        target_actions = action_maps[torch.arange(batch_size), target_frames_indices]
        ...
        reference_actions = action_maps[torch.arange(batch_size), reference_frames_indices]
        ...
        conditioning_actions = action_maps[torch.arange(batch_size), conditioning_frames_indices]
        # Encode observations to latent codes
        with torch.no_grad():
            self.ae.eval()
            input_frames = torch.stack([target_frames, reference_frames, conditioning_frames], dim=1)
            input_actions = torch.stack([target_actions, reference_actions, conditioning_actions], dim=1)
            if self.config["autoencoder"]["type"] == "ours":
                ...
                flat_input_actions = rearrange(input_actions, "b n c h w -> (b n) c h w")
                flat_latents = self.ae.encode(flat_input_frames)
        flat_states = torch.cat([flat_latents, flat_input_actions], dim = 1)
        latents = rearrange(flat_states, "(b n) c h w -> b n c h w", n=3)
    ...
    def generate_frames(
        ...
        if self.config["autoencoder"]["type"] == "ours":
            ...
            flat_latents = self.ae.encode(flat_input_frames)
            flat_input_actions = rearrange(action_maps, "b n c h w -> (b n) c h w")
            flat_states = torch.cat([flat_latents, flat_input_actions], dim = 1)
            latents = rearrange(flat_states, "(b n) c h w -> b n c h w", n=observations.size(1))
        ...
        if self.config["autoencoder"]["type"] == "ours":
            ...
            reconstructed_observations = self.ae.decode(latents[:,:8,:,:])
        reconstructed_observations = rearrange(reconstructed_observations, "(b n) c h w -> b n c h w", b=b)


        return reconstructed_observations, latents[:,8:,:,:]
