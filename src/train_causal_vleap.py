...
from causal_vleap import Generator as Generator
import pickle
...
class Train(object):
  def __init__(self,gen_config,cfg, strategy: tf.distribute.Strategy):
    ...
    with self.strategy.scope():
     ...
     self.gen=Generator(gen_config,skip=self.cfg["skip"],name="causal_leap")
     self.optimizer_gen= tf.keras.optimizers.Adam(learning_rate=self.cfg['lr'], beta_1=self.cfg['beta1'])
     print("Training Causal-LeAP model with Sarkar et al NeurIPS paper config")
     ...
     self.manager_gen = tf.train.CheckpointManager(self.checkpoint_gen, directory=self.cfg['checkpoint_dir_G'], max_to_keep=100)
     ...
     self.action_Lp0= Ls.recon_loss_l2(batch_size=self.global_batch_size,reduction=tf.keras.losses.Reduction.NONE)
     self.act_kld_loss=Ls.KL_Divergence(batch_size=self.global_batch_size,reduction=tf.keras.losses.Reduction.NONE)
      
      
     self.L_recon_metric= Mean(name="reconst_loss")
     self.kld_metric=Mean(name="KL_div")
     self.L_action_metric= Mean(name="action_loss")
     self.action_kld_metric= Mean(name="action_KLD")
  ...
  def compute_loss(self,input):    # input is a dictionary of predict,gen_vel_map, gt,gt_vel_map,D_real,D_logits_real,D_fake, D_logits_fake )
      ...
      action_Lp0=self.action_Lp0(input['at_predict'],input['at_gt'])
      action_kld=self.act_kld_loss(mu_pred=input['ac_post_mean'],logvar_pred=input['ac_post_logvar'],
                        mu_true=input['ac_prior_mean'],logvar_true=input['ac_prior_logvar'])
      action_loss=action_Lp0+self.cfg['gamma']*action_kld
      ...
      loss={  "KL_div":kld, 'reconst_loss': reconst_loss, 'final_loss': final_loss,
       'action_kld':action_kld,'action_mse': action_Lp0, 'action_loss': action_loss }
  ...
  def train_step(self, input): 
      ...
      with tf.GradientTape() as tape0 :
        ...
        at_predict=output[5]
        ac_posterior_mean=output[6]
        ac_posterior_logvar=output[7]
        ac_prior_mean=output[8]
        ac_prior_logvar=output[9]
        ...
        model_output_dict={'predict': predict,'gt':target ,
                  ...
                  "ac_post_mean":ac_posterior_mean , 'ac_post_logvar':ac_posterior_logvar,
                  "ac_prior_mean":ac_prior_mean, 'ac_prior_logvar':ac_prior_logvar}
  ...
  def val_step(self, input): ## input is a disctionary having input of in_seq,xt,vel_seq,action_past_seq, target): 
    ...
    self.gen.A_prior_rnn.init_hidden_state()
    self.gen.A_posterior_rnn.init_hidden_state()
    ...
    for step in range(self.cfg['past_TS']+self.cfg['future_TS']-1):
      ...
      prior_alpha=self.gen.A_prior_rnn([a_embd,z_t],training=self.cfg['G_train'])
      alpha_mu=prior_alpha[0]
      alpha_logvar=prior_alpha[1]
      alpha_t=prior_alpha[2]

        # z_t=prior_mu_t
      h_target_embd=self.gen.img_encoder(im_seq[:,step+1,...],training=self.cfg['G_train'])
      h_target=h_target_embd[0]
      future_a_embd=self.gen.at_encoder(at_seq[:,step+1,...],training=self.cfg['G_train'])
      ...
      post_at=self.gen.A_posterior_rnn([future_a_embd,post_zt],training=self.cfg['G_train'])
      post_alpha_mu=post_at[0]
      post_alpha_logvar=post_at[1]
      

      h_next=self.gen.prediction_rnn([tf.concat([h_t,a_embd],axis=1),z_t],training=self.cfg['G_train'])
      a_hat_next=self.gen.at_predictor([a_embd,alpha_t],training=self.cfg['G_train'])
  ...
  def reset_metrics(self):
      ...
      self.action_kld_metric.reset_state()
  def update_metrics(self,d):
    ...
    self.action_kld_metric.update_state(d['action_kld'])
  ...
  def reduce_dict(self, d: dict):
    ...
    action_kld=self.strategy.reduce(tf.distribute.ReduceOp.SUM, d["action_kld"], axis=None)
    return [recon_loss,kld,final_loss,action_loss,action_kld]
  @tf.function
  ...
  def custom_loop(self,batched_data,val_data):
    ...
    for epoch in range(ckpt,epochs):
      ...
      # for itr in range(total_itr):  #for dgx
        ...
        action_kld=action_kld+self.action_kld_metric.result()
        itr=itr+1
      ...
      template = ('Epoch: {}, Reconstruction Loss: {}, '
              'KL_divergence: {}, Final_loss: {}, Action_loss: {}, Action KLD: {}')
      
      print(template.format(epoch, recon_loss*1.0/itr,
                                kld_loss*1.0/itr,final_loss*1.0/itr,action_loss*1.0/itr,action_kld*1.0/itr))
      ...
      template = ('Training Epoch: {}, Reconstruction Loss: {}, '
            'KL_divergence: {}, Action_loss: {}, Action KLD: {}')
      print(template.format(epoch, self.L_recon_metric.result(),
                                  self.kld_metric.result(),self.L_action_metric.result(), self.action_kld_metric.result()))
