...
class Generator_post(tf.keras.Model):
  def __init__(self, cfg,skip=False,name="svg",**kwargs):
  ...
  def call(self,input):
    ...
    for step in range(self.cfg['past_TS']+self.cfg['future_TS']-1):
      ...
      z_t_prior=self.prior_rnn(h_t,training=self.cfg['is_train'])
      mu_t=z_t_prior[0]
    ...
    return [predict, posterior_mean,posterior_logvar,prior_mean, prior_logvar] ##tf.stack might be helpful
